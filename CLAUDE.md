...
├── 6_analysis.py               # SC_total construction, H1/H2 regressions (future)
├── 7_report.py                 # Thesis-ready tables and figures (future)
...
│   ├── 1_quality_review.md             # Thesis quality review notes
│   └── 2_master_revision.md            # Master revision tracking
...
└── toolkits/                   # Data processing and analysis toolkits
    ...
    ├── event_selection_toolkit.py        # 3-stage shock identification; SC raw components, PCA scoring, CAR persistence
    ├── visualization_toolkit.py          # Bloomberg-style price charts + Shock Score dashboard PNGs
    ├── news_processor_toolkit.py         # FinBERT init, headline selection, Claude API summaries, cache
    └── survey_design_toolkit.py          # Latin square design, manifest population, Markdown QC report
```
...
- **Response collection**: Google Sheets linked per form; merged post-collection with block/version identifiers
