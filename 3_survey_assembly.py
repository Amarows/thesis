...
def generate_report(
    ...
    lines: list[str] = [
        ...
        "",
        "## 1. Scenario Counts",
        f"- **Total scenarios:** {len(manifest_df)}",
    ]
    ...
    lines += [
        ...
        f"**{pca_info.get('explained_variance', 0):.4f}** "
        f"({pca_info.get('explained_variance', 0) * 100:.1f}%)",
        "- Loadings (w_1):",
    ]
    for comp, val in pca_info.get("loadings", {}).items():
    ...
    if not sc_df.empty and "sc_total" in sc_df.columns:
        ...
        for proto in ["Standard Process", "Enhanced Review",
                      "Cooling-Off and Second Review"]:
            n_p = (sc_df["protocol_recommendation"] == proto).sum()
            lines.append(f"- {proto}: {n_p} scenarios")
    ...
    lines += [
        ...
        "set to `[REQUIRES POST-EVENT DATA]`. Extend price data coverage and "
        "re-run the script.",
        "",
    ...
    # File manifest
    lines += ["", "## 6. Generated File Manifest"]
    for dirpath, dirnames, filenames in os.walk(output_dir):
        dirnames.sort()
        for fname in sorted(filenames):
            if fname == "survey_assembly_report.md":
                continue  # report not yet written at scan time
            fpath = Path(dirpath) / fname
            size_kb = fpath.stat().st_size / 1024
            rel = fpath.relative_to(output_dir)
            lines.append(f"- `survey/{rel}` ({size_kb:.1f} KB)")

    n_total = len(manifest_df)
    lines += [
        "",
        "## 7. Completion Checklist",
        f"- [x] `scenario_metadata.csv`",
        f"- [x] `scenario_news_text.csv`",
        f"- [x] `scenario_price_reaction.csv`",
        f"- [x] `scenario_shock_score.csv`",
        ...
        f"- [x] `counterbalancing_matrix.csv`",
        f"- [x] `form_assembly_guide.csv`",
    ]
